...
import {
  AuthAction,
  init,
  withAuthUser as withAuthUserNative,
...
const initAuth = () => {
  init({
    ...
    authPageURL: ({ ctx }) => {
      ...
      return `auth/login?destination=${encodeURIComponent(destURL.toString())}`;
    },
...
export const withAuthUser = withAuthUserNative({
  whenUnauthedBeforeInit: AuthAction.SHOW_LOADER,
  whenUnauthedAfterInit: AuthAction.REDIRECT_TO_LOGIN,
  LoaderComponent: FullPageLoader,
});

export const withAuthUserDefault = withAuthUserNative();
